...
def pr2_mover(object_list):
    ...
    for object in object_list:
        detected_objects_dict.update({object.label:object.cloud})
        points_arr = ros_to_pcl(object.cloud).to_array()
        centroids.update({object.label:np.mean(points_arr, axis=0)[:3]})
    ...
    for i in range(0, len(object_list_param)):
        ...
        # if the object in the pick list is not detected, continue to the next object
        ...
        if object_list_param[i]['group']=='red':
            arm_name.data = 'left'
            place_pose.position.x = dropbox_left[0] - 0.2 + i*0.05
        ...
        # Surpass the following pick and place part. For extra challenges, comment continue
        continue
